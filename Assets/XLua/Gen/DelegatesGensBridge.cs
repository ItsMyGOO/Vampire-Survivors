...
namespace XLua
{
    public partial class DelegateBridge : DelegateBridgeBase
    {
		...
		public bool __Gen_Delegate_Imp13()
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public override Delegate GetDelegateByType(Type type)
		{
		    ...
		    if (type == typeof(Lua.IntBinaryOp))
			{
			    return new Lua.IntBinaryOp(__Gen_Delegate_Imp11);
			}
		
		    if (type == typeof(Lua.LuaTick))
			{
			    return new Lua.LuaTick(__Gen_Delegate_Imp12);
			}
		
		    if (type == typeof(Lua.Condition))
			{
			    return new Lua.Condition(__Gen_Delegate_Imp13);
			}
